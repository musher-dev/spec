...
7. **Exact-version URLs are rebuilt from tags, never from `main`.**
   `tools/src/publication/site.ts` extracts each release from its own tag. Nothing in the
   working tree may feed a path that has already been published. The whole
   published tree is generated by that one script — `_headers`, the index pages
   and the `/reference/` tree included — so the cache policy of a path cannot
   drift from the path. A reference page is built from the bytes its own version
   serves, read at that version's ref, so a released family's page describes its
   release. `_headers` rules must never overlap on a header name; the build
   enforces it. See docs/adr/0012 and docs/adr/0017.
...
`tools/src/schema/lint.ts` — do not work around them. What a **field**, a **value**, or
...
The reference published at `/reference/` is generated from that prose and from
the bundle. It is informative — exactly as each `spec.md`'s own normative-status
note already says generated documentation is — and where it differs from the
Markdown, the Markdown is right and `tools/` has a defect.
